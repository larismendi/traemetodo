...
if(isset($_POST['email'])){
	...
	$obj = new Registros();
	$id = $obj->valida($_POST['email']);
	if($id){
		$obj = new Registros();
		$obj->enviar($obj->datos($id), $id);
		echo 'Se envío el activador satisfactoriamente.';
	}else{
		echo false;
	}
	exit;
...
if(isset($_REQUEST['token'])){
	...
	if($id){
		$obj = new Registros();
		if($obj->alta_email($id)){
			$obj = new Registros();
			$obj->enviar_alta($id);
			$txt = 'Se ha validado satisfactoriamente su email. Gracias por registrarse en www.traemetodo.com -';
		}else{
			$errores = true;
		...
		$errores = true;
		$remail = true;
		$txt = 'Se ha presentado un problema validando su registro. Favor ingresar su correo para enviar nuevamente la verificación de su cuenta vía email.';
	}
}else{
...
label.error { color: red; font-weight: bold; margin-bottom: 10px; padding-bottom: 10px;}
...
<div class="contenido_med">
	...
	<div id="form-main">
	  	<div id="form-div">
		    <form class="form" id="form1" methode="post">
		      <p class="email">
		        <input name="email" type="text" class="required email feedback-input" id="email" placeholder="Email" />
		      </p>
		      ...
		      <div id="mensaje_return">Recupere el validador de email en su correo registrado.</div>
		    </form>
	  	</div>
	</div>
	<? } ?>
...
$(function(){
	$("#form1").validate({
		submitHandler: function(form){
			$.post("registro_token.php", $("#form1").serialize(), function(data){
				if(data){
					$('#form1').each(function(){ this.reset(); });
					$("#mensaje_return").html("<span style='color:green;'>"+data+"</span>");
				}else{
					$("#mensaje_return").html("<span style='color:green;'>No se envío correctamente su información.</span>");
				}
			});
			return false;
		}
