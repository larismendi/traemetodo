...
class Authorization
{
	...
	{
		if(empty($_SESSION['exp_user']) || !isset($_SESSION['exp_user']['autentificado_']) || $_SESSION['exp_user']['autentificado_']!=true || @$_SESSION['exp_user']['expires'] < time()){
			return false;
		}
		else{
			return true;
	...
	public function signin($u,$p)
	{
		...
		if (USEDB){
			if($u&&$p){
				...
				switch(LOGIN_METHOD){
					case 'both':
						$sql .= "(username='".mysql_real_escape_string($u)."' OR email='".mysql_real_escape_string($u)."')";
						break;
					case 'email':
						$sql .= "email='".mysql_real_escape_string($u)."'";
						break;
					default:
				...
				if(mysql_num_rows($rs)){
						$this->set_session(array_merge(mysql_fetch_assoc($rs),array('autentificado_'=>true,'expires'=>time()+(45*60))));
						$return = true;
					}
			...
			for($i=0;$i<count($user_config);$i++){
				switch(LOGIN_METHOD){
					  case 'both':
						  if(($user_config[$i]['username']==$u||$user_config[$i]['email']==$u) && 
							  $user_config[$i]['password']==md5($p)) { $return = true;}
						  break;
					  case 'email':
				...
				if($return===true){
					$this->set_session(array_merge($user_config[$i],array('autentificado_'=>true,'expires'=>time()+(45*60))));
				}
			}
		}			
		return $return;		
