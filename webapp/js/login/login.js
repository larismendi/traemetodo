...
$(document).ready(function(){ 
	...
	var jsErrMsg	=	'Usuario o contraseña inválida';						// clientside error message
	
	var postFile	=	'../../controlador/login/login.post.php';	// post handler
	...
	$(waitId).html(waitNote).fadeIn('fast',function(){
		...
		$.getJSON(postFile, function(data){
			...
			if(data.status==true) {
				...
				if(autoRedir){ 
					$(waitId).hide().html('Iniciando...').fadeIn('fast', function(){window.location=data.url;});
				} else {
									...
									if(_p.length<4)
									...
									else
										{
											$.post(postFile, { u: _u, p: _p }, function(data) {
												if(data.status==true){ 
													if(autoRedir){ 
														$(waitId).html('Iniciando...').fadeIn('fast', function(){
